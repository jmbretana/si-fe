...
import MonitorHeartBit from "./MonitorHeartBit";
...
interface MonitorHeartProps {
  dataLoad: controlDataFc;
  hourMinutes: string;
  //
}
...
const MonitorHeart: React.FunctionComponent<MonitorHeartProps> = (props) => {
  const snd = new Audio(
  ...
  useEffect(() => {
    if (props.dataLoad && beep && props.dataLoad.fc) {
      const fc = props.dataLoad.fc;
      const seconds = Math.trunc((60 / fc) * 1000);
  ...
  const beepHandler = () => {
    beep ? setBeep(false) : setBeep(true);
  };
